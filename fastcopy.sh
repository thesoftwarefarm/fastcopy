#!/usr/bin/env bash
# fast-copydb.sh (v2.5) — apt mysqlsh only; safe SQL; drop/recreate target schema
# Usage: ./fast-copydb.sh /path/to/server.cfg
set -Eeuo pipefail

# Force a safe locale for mysqlsh (avoids noisy warnings)
...
find_free_port(){
  for port in $(shuf -i 24000-29999 -n 200); do
    if command -v ss >/dev/null 2>&1; then ss -lnt "( sport = :$port )" | grep -q "$port" || { echo "$port"; return 0; }
    else lsof -i TCP:"$port" -sTCP:LISTEN >/dev/null 2>&1 || { echo "$port"; return 0; }
    fi
  done; return 1
}
...
# Properly quote a schema identifier with backticks; doubles inner backticks.
qi() {
  local bt=$'\x60'        # backtick character
  local s="${1//${bt}/${bt}${bt}}"
  printf '%s%s%s' "$bt" "$s" "$bt"
}
...
# perf knobs
DUMP_THREADS="${DUMP_THREADS:-$( (command -v nproc >/dev/null && nproc) || echo 4 )}"
DUMP_COMPRESSION="${DUMP_COMPRESSION:-zstd}"     # zstd|gzip|none
LOAD_THREADS="${LOAD_THREADS:-$( (command -v nproc >/dev/null && nproc) || echo 4 )}"
DEFER_INDEXES="${DEFER_INDEXES:-all}"            # none|fulltext|all
IGNORE_EXISTING="${IGNORE_EXISTING:-true}"       # true|false
KEEP_DUMP="${KEEP_DUMP:-false}" # delete dump after successful load? (default: false)
...
if [[ "${DROP_TARGET_DATABASE_BEFORE_LOAD,,}" == "true" ]]; then
  if [[ "$SOURCE_DB_NAME" == "$TARGET_DB_NAME" ]]; then
    ...
    [[ -n "$TARGET_DB_CHARSET"   ]] && ddl+=" CHARACTER SET ${TARGET_DB_CHARSET}"
    [[ -n "$TARGET_DB_COLLATION" ]] && ddl+=" COLLATE ${TARGET_DB_COLLATION}"
    ddl+=";"
    sql "$ddl"
  fi
fi
...
# --- LOAD ---
JS_IGNORE=$([[ "${IGNORE_EXISTING,,}" == "true" ]] && echo true || echo false)
PY_IGNORE=$([[ "${IGNORE_EXISTING,,}" == "true" ]] && echo True || echo False)
